...
version(OSX) static char[32]     g_GlslVersionString = "#version 330";
else static char[32]     g_GlslVersionString = "#version 330";
...
static: 
    ...
    bool init(const (char)* glsl_version)
    {
        ...
        if (g_GlVersion >= 320)
        {
            // Backend flags are set through the C API in imgui_helper when needed.
        }
    ...
    {
        ImFontAtlas* fonts = mv_imgui_font_atlas();
        ...
        ImFontAtlas_GetTexDataAsRGBA32(fonts, &pixels, &width, &height, null);
        ...
        mv_imgui_font_set_tex_id(cast(void*)cast(int*)g_FontTexture);
